...
ageData %>% group_by(AgeBins) %>% summarise(meanSurv = mean(Survived)) %>% ggplot() + geom_col(aes(x = AgeBins, y = meanSurv)) + coord_flip()
```
...
```{r}
ageData %>% filter(Sex == "male") %>% group_by(AgeBins) %>% summarise(meanSurv = mean(Survived)) %>% ggplot() + geom_col(aes(x = AgeBins, y = meanSurv)) + coord_flip() + ggtitle("Survival Rates for Males")
```
...
```{r}
ageData %>% filter(Sex == "female") %>% group_by(AgeBins) %>% summarise(meanSurv = mean(Survived)) %>% ggplot() + geom_col(aes(x = AgeBins, y = meanSurv)) + coord_flip() + ggtitle("Survival Rates for Females")
```
...
fareData %>% group_by(FareBins) %>% summarise(meanSurv = mean(Survived)) %>% ggplot() + geom_col(aes(x = FareBins, y = meanSurv)) + coord_flip()
```
...
```{r}
train %>% group_by(Embarked) %>% summarise(meanSurv = mean(Survived)) %>% filter(Embarked != "") %>% ggplot() + geom_col(aes(x = Embarked, y = meanSurv)) + coord_flip()
```
