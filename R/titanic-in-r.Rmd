...
I have ignored a lot of other common preprocessing steps while cleaning the data and engineering additional features. This is because the caret package takes care of a lot of the necessary preprocessing steps such as dummy variable creation, scaling, centering, and principal components analysis.
...
clean_training$Survived <- factor(clean_training$Survived) # Convert Survied to a factor so that the predictions are discrete.
...
rf_model <- train(Survived ~ ., method = "rf", preProcess = c("knnImpute"), data = clean_training, na.action = na.pass)
```
...
gbm_model <- train(Survived ~ ., method = "gbm", preProcess = c("knnImpute"), data = clean_training, na.action = na.pass)
```
...
svmlinear_model <- train(Survived ~ ., method = "svmLinearWeights", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
svmradial_model <- train(Survived ~ ., method = "svmRadialWeights", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
logit_model <- train(Survived ~ ., method = "LogitBoost", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
lda_model <- train(Survived ~ ., method = "lda", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
adabag_model <- train(Survived ~ ., method = "AdaBag", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
gam_model <- train(Survived ~ ., method = "gam", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
evtree_model <- train(Survived ~ ., method = "evtree", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
nnet_model <- train(Survived ~ ., method = "nnet", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
nb_model <- train(Survived ~ ., method = "nb", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```

Stacked Model (using a Random Forest):
```{r, echo = FALSE}
set.seed(636098)

create_stacked_df <- function(data) {
  rf_pred <- predict(rf_model, data, na.action = na.pass)
  gbm_pred <- predict(gbm_model, data, na.action = na.pass)
  svmlinear_pred <- predict(svmlinear_model, data, na.action = na.pass)
  svmradial_pred <- predict(svmradial_model, data, na.action = na.pass)
  logit_pred <- predict(logit_model, data, na.action = na.pass)
  lda_pred <- predict(lda_model, data, na.action = na.pass)
  adabag_pred <- predict(adabag_model, data, na.action = na.pass)
  gam_pred <- predict(gam_model, data, na.action = na.pass)
  evtree_pred <- predict(evtree_model, data, na.action = na.pass)
  nnet_pred <- predict(nnet_model, data, na.action = na.pass)
  nb_pred <- predict(nb_model, data, na.action = na.pass)
  
  stacked <- tibble(rf_pred, gbm_pred, svmlinear_pred, svmradial_pred, logit_pred, lda_pred,
                    adabag_pred, gam_pred, evtree_pred, nnet_pred, nb_pred)
  return(stacked)
} 

stacked_training <- bind_cols(create_stacked_df(clean_training), tibble(Survived = clean_training$Survived))
stacked_model <- train(Survived ~ ., method = "rf", data = stacked_training)
stacked_predictions <- predict(stacked_model, create_stacked_df(clean_testing), na.action = na.pass)


submission <- tibble(PassengerId = testing$PassengerId, Survived = stacked_predictions)
