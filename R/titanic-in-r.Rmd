...
tan_model <- train(Survived ~ ., method = "tan", preProcess = c("knnImpute", "scale", "center"), data = clean_training, na.action = na.pass)
```
...
create_stacked_df <- function(data) {
  ...
  tan_pred <- predict(tan_model, data, na.action = na.pass)
  
  stacked <- tibble(rf_pred, gbm_pred, svmlinear_pred, svmradial_pred, logit_pred, lda_pred,
                    adabag_pred, gam_pred, evtree_pred, nnet_pred, tan_pred)
  return(stacked)
} 
...
submission <- bind_cols(testing$PassengerId, tibble(stacked_predictions)) 
